export default async function handler(req, res) {
  ...
  try {
    ...
    // AI annab alati täis (PRO) vastuse
    ...
    // 🔥 Tier loogika (preview süsteem)
    let finalAnswer = fullAnswer;

    if (tier === "free") {
      finalAnswer =
        fullAnswer.substring(0, Math.floor(fullAnswer.length * 0.25)) +
        "\n\n🔒 Unlock more details with Starter or PRO";
    } else if (tier === "starter") {
      finalAnswer =
        fullAnswer.substring(0, Math.floor(fullAnswer.length * 0.55)) +
